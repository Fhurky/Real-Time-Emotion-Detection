...
class ModelPredictor:
    def __init__(self, model_path, JSON_path):
        ...
        config_path = os.path.join(self.JSON_path, "configs.json")
    ...
    def extract_feature(self, file, mfcc=True):
        # Yükleme ve ses verisini alma
        X, sample_rate = librosa.load(file, sr=None)
        
        result = np.array([])  # result'ı başlatmak

        if mfcc:
            # MFCC özelliklerini çıkar
            mfccs = np.mean(librosa.feature.mfcc(y=X, sr=sample_rate, n_mfcc=40).T, axis=0)
            result = np.hstack((result, mfccs))  # MFCC'leri 'result' ile birleştir
    ...
    def predict(self, file):
        ...
        """
        X = self.extract_feature(file=file, mfcc=True)
