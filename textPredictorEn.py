...
class TextPredictorEN:
    def __init__(self, device: str = 'cpu', decision=0):
        # Seçilen cihazı kullanarak model pipeline'ı başlatıyoruz
        self.device = device
        self.decision = decision
        self.classifier = pipeline("text-classification", model='bhadresh-savani/bert-base-uncased-emotion', return_all_scores=True, device=0 if device == 'cuda' else -1)
    ...
    def calculate_eng(self, emotions):
        ...
        if anger > 0.30 and sad > 0.30:
            ...
            natural = round(((1 * sad) / 8 + (1 * happy) / 10 + (1 * anger) / 10 + (1 * fear) / 10 + (1 * surprised) / 10) * 100, 2)
            sad = round((7*sad / 8) * 100, 2)
            happy = round((9*happy / 10) * 100, 2)
            anger = round((9*anger / 10) * 100, 2)
            fear = round((9*fear / 10) * 100, 2)
            surprised = round((9*surprised / 10) * 100, 2)
...
# predictor = TextPredictorEN(device='cpu') or # device='cuda'
# text = "I am very happy today!"
# result = predictor.predict_and_calculate(text, decision=0)  # decision=0 çünkü metin Türkçe

# print("Duygu tahmin sonuçları:", result)